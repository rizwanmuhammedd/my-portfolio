...
const ROW_C = ["Git","GitHub","Visual Studio","VS Code","Postman","REST API","Swagger","SignalR","npm","JWT Auth","Figma"];
...
export default function Skills() {
  ...
  useEffect(() => {
    ...
    mm.add("(min-width: 769px)", () => {
      /* ── Pinned horizontal scrub ──
         - outer is the tall scroll space
         - inner pins to viewport
         - track moves left, tied 1:1 to scroll via onUpdate
         scrub: 1.2 = slight lag makes it feel silky smooth
      */
  ...
  return (<>
    <style>{`
      ...
      /* ═══════════════════════════════════
         INNER — viewport-pinned container
      ═══════════════════════════════════ */
      .sk-inner {
        width: 100%;
        height: 100vh;
      ...
      .sk-chip {
        ...
        font-size: 13px; font-weight: 800; letter-spacing: .2em;
        text-transform: uppercase; color: var(--td);
        padding: 0 22px; display: inline-flex; align-items: center;
        gap: 10px; white-space: nowrap; flex-shrink: 0; cursor: default;
        transition: color .2s;
      }
      .sk-chip:hover { color: var(--ac); }
      .sk-chip-dot { color: var(--ac-dim); font-size: 7px; }

      /* Progress bar */
      .sk-prog-bar {
        flex-shrink: 0; height: 2px; background: var(--border);
      }
      #sk-fill {
        height: 100%; background: var(--ac);
      ...
      /* ═══════════════════════════════════
         TRACK — all skill panels in a row
      ═══════════════════════════════════ */
      .sk-track {
        display: flex;
        flex: 1;
      ...
      /* ═══════════════════════════════════
         PANEL — one skill category = 100vw
      ═══════════════════════════════════ */
      .sk-panel {
        flex-shrink: 0;
        width: 100vw;
      ...
      .sk-card:hover .sk-title { color: var(--ac); }
      .sk-tags { display: flex; flex-wrap: wrap; gap: 7px; margin-top: 4px; }
      .sk-tag {
        font-family: 'Barlow Condensed', sans-serif;
        font-size: 11px; font-weight: 700; letter-spacing: .13em;
        text-transform: uppercase; color: var(--td);
        border: 1px solid var(--border); padding: 6px 13px; transition: all .18s;
      }
      .sk-card:hover .sk-tag { color: var(--ac-dim); border-color: var(--border-ac); }
      ...
      .sk-bar-fill {
        height: 100%; background: var(--ac);
        box-shadow: 0 0 10px rgba(200,241,53,.4);
        /* Animate via CSS custom property */
        width: 0;
        transition: width 1.4s cubic-bezier(.25,.46,.45,.94) .2s;
      }
      html.light .sk-bar-fill { box-shadow: 0 0 8px rgba(200,21,27,.3); }

      /* Trigger bar animation when card enters viewport (desktop scroll) */
      .sk-panel:has(.sk-card:hover) .sk-bar-fill,
      .sk-bar-fill.animate {
        /* fallback — bars always fill after a delay on desktop */
      }
      ...
      .sk-bounce-arrow {
        color: var(--ac-dim);
        animation: sk-bounce 1.8s ease-in-out infinite;
      }
      @keyframes sk-bounce {
      ...
      @media (max-width: 480px) {
        ...
        .sk-panel { padding: 14px 14px 22px; }
      }
    `}</style>
      ...
      <section ref={innerRef} className="sk-inner">
              ...
              <div className="sk-mq-track">
                {[0, 1].map(s => row.map((t, i) => (
                  <span key={`${s}-${i}`} className="sk-chip">
                    {t}<span className="sk-chip-dot">✦</span>
                  </span>
                )))}
              </div>
            </div>
        ...
        <div ref={trackRef} className="sk-track">
          {SKILLS.map((cat) => {
            const { Icon } = cat;
            return (
              <div key={cat.id} className="sk-panel">
                      ...
                      <div key={b.n} className="sk-prow">
                        ...
                        <div className="sk-bar-track">
                          <div
                            ...
                            data-p={b.p}
                            /* Bars fill after mount — CSS transition handles the animation */
                            ref={el => {
                              if (el) setTimeout(() => { el.style.width = b.p + "%"; }, 600);
                            }}
                          />
                        </div>
                      </div>
        ...
        <div className="sk-footer">
          <div className="sk-footer-txt">
            Scroll slowly to explore each category
            <span className="sk-bounce-arrow">→</span>
          </div>
          <div className="sk-panel-count">
            <span className="sk-count-ac">{SKILLS.length}</span> Categories
          </div>
        </div>
